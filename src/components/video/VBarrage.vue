<template>
  <section ref="barrageWrapper" class="barrage-wrapper" :style="{height:percent+'%'}">
    <div class="barrage-main">
      <div ref="barrageMainDm" class="barrage-main-dm" :class="{'ani-pause':isPause,'ani-running':!isPause}"></div>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    shootTopDanmu(dmItem, channel) {
      ...
      dom.style.transform =
          `translate3d(${this.barMainWidth / 2 - (dom.clientWidth / 2)}px,${channel * dom.clientHeight}px,0)`
